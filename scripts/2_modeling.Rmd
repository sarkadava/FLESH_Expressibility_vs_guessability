...
The cosine similarity was calculated with ConceptNet. Some pairs (N = 130, ~2% of the data) were not available in ConceptNet and for those we asked participants (HOW MANY) to rate the perceived similarity. On a smaller set of pairs, we previously checked whether this is an effective way to reduce the missing values, and we found that ConceptNet and perceived similarity ratings are highly correlated (r = ????). The data reviewed below already entails the perceived similarity.

Cosine similarity is inherently on -1 to 1 scale.
As provided
...
Meaning that a negative value is provided for concepts that are clearly different (i.e., not confusable).

In order to be able to model the values with a wider choice of families (including zero-one inflated beta), we transformed the cosine similarity with

`(df$cosine_similarity + 1) / 2`

Below, we break down the missing values and look at data distribution to choose an appropriate family.
...
another peak around 0.15 which results from non-ideal guesses.
...
Estimated posterior binary guessability is `r plogis(fixef(mdl_null_binary)[1])`.
...
mdl_null_similarity <- brm(
  # Main formula for the beta part (μ)
  bf(cosine_similarity ~ 1 + (1 | dyad/participant_ID) + (1 | concept)) +
  # Explicit intercept-only subformulas for the other parameters:
  bf(phi ~ 1) +
  bf(zoi ~ 1) +
  bf(coi ~ 1),  # coi is fixed at 1, so set this via a constant.
  data = df,
  family = zero_one_inflated_beta(),
  prior = c(
    # For the main mu parameter:
    prior(normal(0, 5), class = "Intercept", resp = "cosine_similarity"),
    prior(normal(0, 2), class = "sd", resp = "cosine_similarity"),
    # Priors for the other distributional parameters:
    prior(normal(0, 5), class = "Intercept", dpar = "phi"),
    prior(normal(0, 5), class = "Intercept", dpar = "zoi"),
    # to fix coi to 1, set a very tight prior:
    prior(normal(1, 0.01), class = "Intercept", dpar = "coi")
  ),
  cores = 4,
  chains = 4,
  iter = 10000,
...
mdl_null_similarity <- readRDS(paste0(models, "mdl_null_similarity.rds"))
```
...
mdl_null_similarity_R2 <- readRDS(paste0(models, "mdl_null_similarity_R2.rds"))

mdl_null_similarity_R2
```
...
Estimated posterior similarity guessability is `r back_transform_zoib(fixef(mdl_null_similarity, dpar = "mu")["Intercept", "Estimate"], fixef(mdl_null_similarity, dpar = "zoi")["Intercept", "Estimate"])`.
...
```{r}
mdl_H1_binary <- brm(
  formula = guess_binary ~ expressibility_dutch + (1 | dyad/participant_ID) + (1 | concept),
  data = df,
  family = bernoulli(link = "logit"),
  prior = c(
    prior(normal(0, 2.5), class = "Intercept"),
...
mdl_H1_similarity <- brm(
  formula = cosine_similarity  ~ expressibility_dutch + (1 | dyad/participant_ID) + (1 | concept),
  data = df,
  family = zero_one_inflated_beta(),
  # Priors: using weakly informative priors on the intercept and slope,
  # plus a beta(1,1) prior for the zero-one inflation parameter (zoi)
  prior = c(
    prior(normal(0, 2.5), class = "Intercept"),
    prior(normal(0, 1), class = "b"),        # Prior for expressibility
    prior(normal(0, 2), class = "sd"),
...
```{r}
model_H2_poisson <- brm(
  formula = N_repetition ~ expressibility + (1 | dyad/participant_ID) + (1 | concept),
  data = df_H2,
  family = poisson(),
  prior = c(
    prior(normal(0, 2.5), class = "Intercept"),
...
# if we need to compress the model more
#saveRDS(model_H2_poisson, file = paste0(models, "model_H2_poisson.rds"), compress = "xz")

model_H2_poisson <- readRDS(paste0(models, "model_H2_poisson.rds"))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Calculate Bayesian R²
model_H2_poisson_R2 <- bayes_R2(model_H2_poisson)
# Save the R² output
saveRDS(model_H2_poisson_R2, file = paste0(models, "model_H2_poisson_R2.rds"))

model_H2_poisson_R2 <- readRDS(paste0(models, "model_H2_poisson_R2.rds"))
...
If you detect overdispersion (variance >> mean), consider a negative binomial model.
...
```{r}
mdl_H3_binary <- brm(
  formula = guess_binary ~ exp_part + (1 | dyad/participant_ID) + (1 | concept),
  data = df,
  family = bernoulli(link = "logit"),
  prior = c(
    prior(normal(0, 2.5), class = "Intercept"),
    prior(normal(0, 1), class = "b"),        # Prior for exp_part
    prior(normal(0, 2), class = "sd")
  ),
  cores = 4,
  chains = 4,
...
mdl_H3_similarity <- brm(
  formula = cosine_similarity  ~ exp_part + (1 | dyad/participant_ID) + (1 | concept),
  data = df,
  family = zero_one_inflated_beta(),
  # Priors: using weakly informative priors on the intercept and slope,
  # plus a beta(1,1) prior for the zero-one inflation parameter (zoi)
...
```{r}
mdl_H4_binary <- brm(
  formula = guess_binary ~ modality + (1 | dyad/participant_ID) + (1 | concept),
  data = df,
  family = bernoulli(link = "logit"),
  prior = c(
    prior(normal(0, 2.5), class = "Intercept"),
...
mdl_H4_similarity <- brm(
  formula = cosine_similarity  ~ modality + (1 | dyad/participant_ID) + (1 | concept),
  data = df,
  family = zero_one_inflated_beta(),
  # Priors: using weakly informative priors on the intercept and slope,
  # plus a beta(1,1) prior for the zero-one inflation parameter (zoi)
