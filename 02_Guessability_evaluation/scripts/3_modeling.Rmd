...
This script uses the output of `2_descriptive.Rmd` and produces all the
...
# Demographics
demo <- read_delim(paste0(dataset, "demo.csv"), delim = "\t")
...
```{r}
kept_ids <- unique(paste(df$dyad, df$participant_dyad, sep = "_"))
# verify the match first
sum(demo$pcn_ID %in% kept_ids)    # how many demo rows match
setdiff(demo$pcn_ID, kept_ids)    # demo participants to be dropped

demo <- demo %>%
  filter(pcn_ID %in% kept_ids)
```
...
# Age and gender
summary(demo$age)  # Shows min, max, mean, median, quartiles
table(demo$gender)  # Shows gender counts
```

There are `r length(unique(df$participant_ID))` speakers from `r length(unique(df$dyad))` dyads. 
...
print(loo_compare(loo_nested_prior, loo_nested, loo_simple))
...
grid.arrange(
  ...
  mdl_null_binary %>%
    ...
    filter(is.finite(condition_mean)) %>%  # Remove infinite/NA values
    ...
    ggtitle("1 | concept") + xlab("Intercept") + ylab(NULL) + 
    # Remove xlim to let ggplot auto-scale
    theme(legend.position="bottom"),
  nrow=1
)
```
...
ggsave(paste0(plots, "H1_cosineModel.png"), plot = g, width = 13, height = 7, dpi = 300)
```
...
ggsave(paste0(plots, "H2_model.png"), plot = last_plot(), width = 8, height = 3, dpi = 300)
...
```{r echo=FALSE, fig.cap=capFig("Posterior estimates of the random effects showing the median (dot), 50% (thick line) and 95% (thin lines) quantiles. Also showing 0 as the vertical dotted line and negative (blue) vs positive (red) values."), fig.height=1*7, fig.width=1*4, warning=FALSE, results='hide'}
...
The variance across different concepts is much larger than the variance across different dyads.

We can plot the variance across the concepts extracting draws.

```{r echo=FALSE, fig.cap=capFig("Posterior binary guessability of concepts showing the median (dot), 50% (thick line), 80% (medium thick line), and 95% (thin lines) quantiles."), fig.height=1*7, fig.width=4, warning=FALSE, results='hide'}
mdl_H2_poisson %>%
  spread_draws(r_concept[concept]) %>%
  # Combine the fixed intercept with the concept-specific random effect
...
```{r echo=FALSE, fig.cap=capFig("Raw binary guessability of concepts showing the median (dot), 50% (thick line), 80% (medium thick line), and 95% (thin lines) quantiles."), fig.height=1*7, fig.width=4, warning=FALSE, results='hide'}
df_part2 %>% 
...
(This part problematic to knit (reason unknown); therefore, it is not evaluated or included in the html but still works fine in the Rmd. One just needs a powerful computer or lots of patience.)
...
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Fixed effects."), results='hide', warning=FALSE}
plot_model(mdl_H3_binary, type="emm", terms=c("exp_part"));
```
```{r}
# because of the logit link, it needs to be backtransformed
...
ggsave(paste0(plots, "H3_models.png"), plot = g, width = 12, height = 4, dpi = 300)
```
...
```{r fig.width=2*4, fig.height=1*7, fig.cap=capFig("Left: Raw mean binary guessability by modality. Right: Model-predicted probability of a correct guess by modality. The plot shows the median predicted probability along with uncertainty bands."), results='hide', warning=FALSE}
# left panel
p_obs <- ggplot(df, aes(x = modality, y = guess_binary, fill = modality)) +
  ...
  labs(x = "Modality", y = "Mean binary guessability") +
  scale_fill_manual(values = c("combined" = colorBlindBlack8[1],
                               "gesture"  = colorBlindBlack8[2],
                               "vocal"    = colorBlindBlack8[3])) +
  guides(fill = "none") +
  scale_y_continuous(breaks = seq(0, 0.6, by = 0.1)) +
  coord_cartesian(ylim = c(0, 0.6)) +          # zoom, don't censor
  theme_minimal(base_size = 18) +
...
# Arrange the two panels vertically using grid.arrange
grid.arrange(p_obs, p_pred, nrow = 1)

#save

combined_plot <- p_obs + p_pred   # panels auto-align

ggsave(paste0(plots, "H4_binaryModel.png"),
       plot = combined_plot, width = 13, height = 4, dpi = 300)
```
...
```{r fig.width=5, fig.height=5, fig.cap=capFig("Posterior predictive density overlay for n=500 draws."), results='hide', warning=FALSE}
pp_check(mdl_H4_similarity, ndraws=500) + xlab('Cosine similarity') + ggtitle('Posterior predictive density overlay');
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Posterior predictive distribution."), results='hide', warning=FALSE}
...
```{r message=FALSE, warning=FALSE, include=FALSE}
if( !file.exists(paste0(models, "mdl_comb_bin_res.RData")) )
{
  # the model:
  mdl_comb_bin <- brm(guess_binary ~ 1 + 
                      ...
                      seed = 17, cores = parallelly::availableCores(), iter = 10000, warmup = 4000, thin = 2,
                      control = list(adapt_delta = 0.99, max_treedepth = 13));
  
  summary(mdl_comb_bin); 
  mcmc_plot(mdl_comb_bin, type="trace"); mcmc_plot(mdl_comb_bin);
  bayestestR::hdi(mdl_comb_bin, ci = 0.95);
  hypothesis(mdl_comb_bin, c("expressibility_dutch > 0", 
                             "exp_part1 > 0", 
                             "modalitygesture < 0", 
                             "modalityvocal < modalitygesture")); 
  # posterior predictive checks:
  pp_check(mdl_comb_bin, ndraws = 100) + xlab('p(guess)') + ggtitle('Posterior predictive density overlay');
  grid.arrange( pp_check(mdl_comb_bin, type = 'stat', sta = 'min') + xlab('p(guess)') + ggtitle('Posterior predictive distribution of minimum values') + xlim(0,1),
                pp_check(mdl_comb_bin, type = 'stat', stat = 'mean') + xlab('p(guess)') + ggtitle('Posterior predictive distribution of means') + xlim(0,1),
                pp_check(mdl_comb_bin, type = 'stat', stat = 'max') + xlab('p(guess)') + ggtitle('Posterior predictive distribution of maximum values') + xlim(0,1),
                ncol = 1);
  
  # Compute Bayesian R² and save it:
  mdl_comb_bin_R2 <- bayes_R2(mdl_comb_bin);
  saveRDS(mdl_comb_bin_R2, file = paste0(models, "mdl_comb_bin_R2.rds"), compress = "xz");
  
  # save the model:
  mdl_comb_bin_res <- mdl_comb_bin;
  saveRDS(mdl_comb_bin_res, paste0(models, "mdl_comb_bin_res.RData"), compress = "xz");
} else {
  mdl_comb_bin_res <- readRDS(paste0(models, "mdl_comb_bin_res.RData"));
  # Load previously saved Bayesian R²:
  mdl_comb_bin_R2 <- readRDS(paste0(models, "mdl_comb_bin_R2.rds"));
}

```

R² is `r mdl_comb_bin_R2[1]` suggesting that `r mdl_comb_bin_R2[1]*100`% of the variance in the outcome variable is explained by the model.

**Diagnostic plots**:

```{r echo=TRUE, message=FALSE, warning=FALSE}
plot(mdl_comb_bin_res)
```
...
```{r}
summary(mdl_comb_bin_res)
```
```{r fig.width=10, fig.height=5, fig.cap=capFig("Trace of the fitting process."), results='hide', warning=FALSE}
mcmc_plot(mdl_comb_bin_res, type="trace");
```
...
```{r fig.width=5, fig.height=5, fig.cap=capFig("Posterior predictive density overlay for n=500 draws."), results='hide', warning=FALSE}
pp_check(mdl_comb_bin_res, ndraws=500) + xlab('p(Guess)') + ggtitle('Posterior predictive density overlay');
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Posterior predictive distribution."), results='hide', warning=FALSE}
grid.arrange(pp_check(mdl_comb_bin_res, type='stat', sta ='min') + xlab('Cosine similarity') + ggtitle('Posterior predictive distribution of minimum values') + xlim(0,1),
             pp_check(mdl_comb_bin_res, type='stat', stat='mean') + xlab('Cosine similarity') + ggtitle('Posterior predictive distribution of means') + xlim(0,1),
             pp_check(mdl_comb_bin_res, type='stat', stat='max') + xlab('Cosine similarity') + ggtitle('Posterior predictive distribution of maximum values') + xlim(0,1),
             ncol=1); 
```
...
```{r fig.width=10, fig.height=10, fig.cap=capFig("Fixed effects' posterior distributions."), results='hide', warning=FALSE}
mcmc_plot(mdl_comb_bin_res);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Fixed effects."), results='hide', warning=FALSE}
plot_model(mdl_comb_bin_res, type="emm", terms=c("expressibility_dutch"));
plot_model(mdl_comb_bin_res, type="emm", terms=c("exp_part"));
plot_model(mdl_comb_bin_res, type="emm", terms=c("modality"));
```
```{r}
# because of the logit link, it needs to be backtransformed
emmeans(mdl_comb_bin_res, ~ expressibility_dutch, type = "response")
emmeans(mdl_comb_bin_res, ~ exp_part, type = "response")
emmeans(mdl_comb_bin_res, ~ modality, type = "response")

pairs(emmeans(mdl_comb_bin_res, ~ exp_part, type = "response"))
pairs(emmeans(mdl_comb_bin_res, ~ modality, type = "response"))
```
```{r}
bayestestR::hdi(mdl_comb_bin_res, ci=0.95)

# experiment part
hypothesis(mdl_comb_bin_res, c("exp_part1 > 0",
                               "(inv_logit_scaled(Intercept + exp_part1)) > (inv_logit_scaled(Intercept - exp_part1))"))

# modality
hypothesis(mdl_comb_bin_res, c("modalitygesture < 0", 
                            "modalityvocal < 0"
                            ))

hypothesis(mdl_comb_bin_res,
  c(# combined better than gesture
    "inv_logit_scaled(Intercept) > inv_logit_scaled(Intercept + modalitygesture)",
    # combined better than vocal
    "inv_logit_scaled(Intercept) > inv_logit_scaled(Intercept + modalityvocal)",
...
**Intercept**:  
The intercept is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_bin_res)["Intercept", "Estimate"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_low"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_high"])` on the log-odds ratio (LOR) scale. When we convert this value using the inverse-logit transformation, it corresponds to an **overall predicted probability** of a correct guess (when **combined** is the reference modality) of `r sprintf("%.1f%% 95%%CI [%.1f%%, %.1f%%]", 100 * plogis(fixef(mdl_comb_bin_res)["Intercept", "Estimate"]), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_low"]), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_high"]))`.  

**Expressibility**:  
The effect of *expressibility* is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_bin_res)["expressibility_dutch", "Estimate"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[2, "CI_low"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[2, "CI_high"])` on the log-odds scale. This substantial positive effect suggests that more **expressible** concepts have a significantly higher probability of being guessed correctly. When moving from **low** expressibility (e.g., **0.0**) to **high** expressibility (**1.0**), the predicted probability of a correct guess increases from about 
`r sprintf("%.1f%% 95%%CI [%.1f%%, %.1f%%]", 100 * plogis(fixef(mdl_comb_bin_res)["Intercept", "Estimate"] + fixef(mdl_comb_bin_res)["expressibility_dutch", "Estimate"] * 0), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_low"] + bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[2, "CI_low"] * 0), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_high"] + bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[2, "CI_high"] * 0))` (**low expressibility**) to `r sprintf("%.1f%% 95%%CI [%.1f%%, %.1f%%]", 100 * plogis(fixef(mdl_comb_bin_res)["Intercept", "Estimate"] + fixef(mdl_comb_bin_res)["expressibility_dutch", "Estimate"] * 1), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_low"] + bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[2, "CI_low"] * 1), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_high"] + bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[2, "CI_high"] * 1))` (**high expressibility**).  

**Experiment part**:  
The effect of *experiment part (feedback = Part 2)* is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_bin_res)["exp_part1", "Estimate"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[3, "CI_low"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[3, "CI_high"])` on the log-odds scale. This positive effect indicates that **providing feedback (Part 2)** increases the probability of a correct guess. When moving from **no feedback (Part 1)** to **feedback (Part 2)**, the predicted probability of a correct guess changes from `r sprintf("%.1f%% 95%%CI [%.1f%%, %.1f%%]", 100 * plogis(fixef(mdl_comb_bin_res)["Intercept", "Estimate"]), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_low"]), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_high"]))` (**Part 1**)  
to  `r sprintf("%.1f%% 95%%CI [%.1f%%, %.1f%%]", 100 * plogis(fixef(mdl_comb_bin_res)["Intercept", "Estimate"] + fixef(mdl_comb_bin_res)["exp_part1", "Estimate"]), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_low"] + bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[3, "CI_low"]), 100 * plogis(bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[1, "CI_high"] + bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[3, "CI_high"]))` (**Part 2**).  

**Modality effects**:  

*Gesture*:  
The effect of **gesture** is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_bin_res)["modalitygesture", "Estimate"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[4, "CI_low"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[4, "CI_high"])` on the log-odds scale.  
Switching from **combined** to **gesture** results in a small, non-significant change in the probability of a correct guess, suggesting **gesture and combined perform similarly**.

*Vocal*:  
The effect of **vocal** is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_bin_res)["modalityvocal", "Estimate"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[5, "CI_low"], bayestestR::ci(mdl_comb_bin_res, ci = 0.95, method = "ETI")[5, "CI_high"])` on the log-odds scale.  
Switching from **combined** to **vocal** results in a **substantial decrease** in the probability of a correct guess, showing that **vocal-only signals are reliably less effective than both combined and gesture modalities**.


```{r fig.width=10, fig.height=10, fig.cap=capFig("Posterior draws for the fixed effects."), results='hide', warning=FALSE}
# Extract posterior draws for the fixed effects
# For our binary model, the coefficients are on the logit scale.
posterior_fixed <- as.matrix(mdl_comb_bin_res, variable = "b_.*", regex = TRUE)
...
```{r fig.width=10, fig.height=10, fig.cap=capFig("Posterior estimates of the fixed effects showing the median (dot), 50% (thick line) and 95% (thin lines) quantiles. Also showing 0 as the vertical dotted line and negative (blue) vs positive (red) values."), results='hide', warning=FALSE}
gather_draws(mdl_comb_bin_res, `b_.*`, regex = TRUE) %>%
    # remove the b_ for plotting
    #mutate(.variable = gsub("b_","", .variable)) %>%
    ggplot(aes(x = .value, y = .variable, fill = after_stat(x < 0))) +
    geom_vline(xintercept=0.0, linetype="dotted", color="gray30") +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
as.data.frame(VarCorr(mdl_comb_bin_res)$concept$sd) %>%
  rownames_to_column(var = "Random Effect") %>%
  select(`Random Effect`, Estimate, Est.Error, Q2.5, Q97.5)

as.data.frame(VarCorr(mdl_comb_bin_res)$dyad$sd) %>%
  rownames_to_column(var = "Random Effect") %>%
  select(`Random Effect`, Estimate, Est.Error, Q2.5, Q97.5)
```
...
mdl_comb_bin_res %>%
  spread_draws(r_concept[concept]) %>%
  # Combine the fixed intercept with the concept-specific random effect
  mutate(mu_concept = fixef(mdl_comb_bin_res)["Intercept", "Estimate"] + r_concept) %>%
  # Back-transform from the logit scale to probabilities
  mutate(guessability = plogis(mu_concept)) %>%
  median_qi(guessability, .width = c(0.5, 0.8, 0.95)) %>%
  ggplot(aes(x = guessability, y = fct_reorder(concept, guessability))) +
...
**Interpretation**: This model tested how guessability is influenced by *expressibility*, feedback availability (*experiment part*), and *modality* (combined, gesture, vocal).

The intercept of −4.46 on the log-odds scale corresponds to an overall predicted probability of a correct guess of about 1.1% in the combined condition (95% credible interval: 0.7%–2.0%) when expressibility is at its lowest and before accounting for other effects.

The effect of expressibility is strongly positive (6.08 log-odds), meaning that more expressible concepts are significantly more likely to be guessed correctly. At low expressibility (0.0), the predicted probability of a correct guess is only about 1% (95% CI: 0.7%–2.0%), but at high expressibility (1.0), the predicted probability rises to approximately 88% (95% CI: 84%–91%). This confirms that expressibility is a key driver of guessability, with a near-100% posterior probability that its effect is positive.

For experiment part, the coefficient of 0.42 indicates that providing feedback (Part 2) increases the probability of a correct guess compared to when no feedback is available (Part 1). Specifically, in Part 1, the predicted probability of a correct guess is about 11% (95% CI: 8%–14%), whereas in Part 2, it increases to approximately 14% (95% CI: 11%–18%). Although this is a relatively small improvement, the 95% credible interval does not include zero, suggesting that feedback has a meaningful effect on improving correctness.

Regarding modality, the coefficient for gesture (0.12) is close to zero, meaning that gesture and combined are nearly identical in guessability. The predicted probability of a correct guess is about 11% in the combined condition (95% CI: 8%–14%) and about 12% in the gesture condition (95% CI: 9%–15%). The posterior probability that combined is better than gesture is low, confirming that there is no strong evidence of a difference.

By contrast, the coefficient for vocalization (−0.69) is negative, indicating that vocal signals are significantly less guessable than both combined and gesture. The predicted probability of a correct guess in the vocal condition is only about 6.5% (95% CI: 5%–8%), meaning that purely vocal productions lead to a much lower chance of success compared to the other two modalities.

Posterior hypothesis tests further confirm these findings: the probability that gesture is different from combined is low (~70%), indicating that gesture and combined yield similar guessability rates, while the probability that vocal is worse than gesture is near 100%, confirming that vocal-only signals significantly reduce correct guesses.

In practical terms, these results show that expressibility is the strongest predictor of guessability, followed by feedback (which offers a modest benefit). Modality plays a crucial role as well: gesture alone is just as effective as a combined signal, while vocal-only communication significantly lowers success rates. This means that using gesture, either alone or in combination with other signals, greatly improves the likelihood of successful communication, while relying solely on vocalization strongly decreases the chance that a concept will be understood correctly.


## Guess similarity

```{r message=FALSE, warning=FALSE, include=FALSE}
if( !file.exists(paste0(models, "mdl_comb_sim_res.RData")) )
{
  # Define the model:
  mdl_comb_sim <- brm(
    # Main formula for the μ (mean) component:
    bf(cosine_similarity  ~ expressibility_dutch + exp_part + modality + 
       (1 | dyad) + (1 | concept),
    # Explicit subformulas for additional parameters:
    phi ~ 1,                           # Precision parameter (controls concentration)
    zoi ~ 1,                           # Zero–one inflation: probability of coming from the beta part
    coi ~ 1),                           # One inflation: probability of an observation being exactly 1
    data = df,
    ...
    prior = c(
      # Priors for the μ component (cosine_similarity):
      prior(normal(0, 2.5), class = "Intercept"),
      prior(normal(0, 1), class = "b"),  # for expressibility_dutch, exp_part, modality
      prior(normal(0, 2), class = "sd")
    ),
    sample_prior = TRUE,  # needed for hypotheses tests
    seed = 17, cores = parallelly::availableCores(), iter = 10000, warmup = 4000, thin = 2,
  ...
  summary(mdl_comb_sim);
  mcmc_plot(mdl_comb_sim, type = "trace");
  mcmc_plot(mdl_comb_sim);
  bayestestR::hdi(mdl_comb_sim, ci = 0.95);
  
  # Hypothesis testing:
  hypothesis(mdl_comb_sim, c("expressibility_dutch > 0",
                                        "exp_part1 > 0",
                                        "modalitygesture < 0",
                                        "modalityvocal < modalitygesture"));
  
  # Posterior predictive checks:
  pp_check(mdl_comb_sim, ndraws = 100) + 
    xlab('Cosine Similarity') + ggtitle('Posterior predictive density overlay');

  grid.arrange( 
    pp_check(mdl_comb_sim, type = 'stat', stat = 'min') + 
      xlab('Cosine Similarity') + ggtitle('Posterior predictive distribution of minimum values') + xlim(0,1),
    pp_check(mdl_comb_sim, type = 'stat', stat = 'mean') + 
      xlab('Cosine Similarity') + ggtitle('Posterior predictive distribution of means') + xlim(0,1),
    pp_check(mdl_comb_sim, type = 'stat', stat = 'max') + 
      xlab('Cosine Similarity') + ggtitle('Posterior predictive distribution of maximum values') + xlim(0,1),
    ncol = 1
  );
  
  
  # Compute Bayesian R² and save it:
  mdl_comb_sim_R2 <- bayes_R2(mdl_comb_sim);
  saveRDS(mdl_comb_sim_R2, file = paste0(models, "mdl_comb_sim_R2.rds"), compress = "xz");
  
  # Save the model:
  mdl_comb_sim_res <- mdl_comb_sim;
  saveRDS(mdl_comb_sim_res, paste0(models, "mdl_comb_sim_res.RData"), compress = "xz");
} else {
  mdl_comb_sim_res <- readRDS(paste0(models, "mdl_comb_sim_res.RData"));
  
  # Load previously saved Bayesian R²:
  mdl_comb_sim_R2 <- readRDS(paste0(models, "mdl_comb_sim_R2.rds"))
}
```

R\^2 is `r mdl_comb_sim_R2[1]` suggesting that `r mdl_comb_sim_R2[1]*100`% of the variance in the outcome variable are explained by the model.

**Diagnostic plots**:

```{r echo=TRUE, message=FALSE, warning=FALSE}
plot(mdl_comb_sim_res)
```
...
```{r}
summary(mdl_comb_sim_res)
```
```{r fig.width=10, fig.height=5, fig.cap=capFig("Trace of the fitting process."), results='hide', warning=FALSE}
mcmc_plot(mdl_comb_sim_res, type="trace");
```
...
```{r fig.width=5, fig.height=5, fig.cap=capFig("Posterior predictive density overlay for n=500 draws."), results='hide', warning=FALSE}
pp_check(mdl_comb_sim_res, ndraws=500) + xlab('Cosine similarity') + ggtitle('Posterior predictive density overlay');
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Posterior predictive distribution."), results='hide', warning=FALSE}
grid.arrange(pp_check(mdl_comb_sim_res, type='stat', sta ='min') + xlab('Cosine similarity') + ggtitle('Posterior predictive distribution of minimum values') + xlim(0,1),
             pp_check(mdl_comb_sim_res, type='stat', stat='mean') + xlab('Cosine similarity') + ggtitle('Posterior predictive distribution of means') + xlim(0,1),
             pp_check(mdl_comb_sim_res, type='stat', stat='max') + xlab('Cosine similarity') + ggtitle('Posterior predictive distribution of maximum values') + xlim(0,1),
             ncol=1); 
```
...
```{r fig.width=10, fig.height=10, fig.cap=capFig("Fixed effects' posterior distributions."), results='hide', warning=FALSE}
mcmc_plot(mdl_comb_sim_res);
```
```{r fig.width=5, fig.height=5, fig.cap=capFig("Fixed effects."), results='hide', warning=FALSE}
plot_model(mdl_comb_sim_res, type="emm", terms=c("expressibility_dutch"));
plot_model(mdl_comb_sim_res, type="emm", terms=c("exp_part"));
plot_model(mdl_comb_sim_res, type="emm", terms=c("modality"));
```
```{r}
# because of the logit link, it needs to be backtransformed
emmeans(mdl_comb_sim_res, ~ expressibility_dutch, type = "response")
emmeans(mdl_comb_sim_res, ~ exp_part, type = "response")
emmeans(mdl_comb_sim_res, ~ modality, type = "response")

pairs(emmeans(mdl_comb_sim_res, ~ exp_part, type = "response"))
pairs(emmeans(mdl_comb_sim_res, ~ modality, type = "response"))
```
```{r}
bayestestR::hdi(mdl_comb_sim_res, ci=0.95)

# experiment part
hypothesis(mdl_comb_sim_res, c("exp_part1 > 0",
                               "(inv_logit_scaled(Intercept + exp_part1)) > (inv_logit_scaled(Intercept - exp_part1))"))

# modality
hypothesis(mdl_comb_sim_res, c("modalitygesture < 0", 
                            "modalityvocal < 0"
                            ))

hypothesis(mdl_comb_sim_res,
  c(# combined better than gesture
    "inv_logit_scaled(Intercept) > inv_logit_scaled(Intercept + modalitygesture)",
    # combined better than vocal
    "inv_logit_scaled(Intercept) > inv_logit_scaled(Intercept + modalityvocal)",
...
```
```{r}
# Create new dataset with all combinations of predictors
newdata_all <- expand.grid(
  expressibility_dutch = seq(0, 1, length.out = 10),  # Expressibility from 0 to 1
  exp_part = c(1, 2),  # Experiment part levels
  modality = c("combined", "gesture", "vocal")  # All modalities
)

# Generate posterior predictions (response scale, ignoring random effects)
post_mod <- add_epred_draws(mdl_comb_sim_res, newdata = newdata_all, re_formula=NA)

# Summarize results for each condition
post_mod_summary <- post_mod %>%
  group_by(expressibility_dutch, exp_part, modality) %>%
  mean_qi(.epred, .width=c(0.95))  # Compute median and 95% CI
```


**Intercept**:  
The model yields an estimated intercept for the β‐component (mu submodel) of  
`r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_sim_res, dpar = "mu")["Intercept","Estimate"], bayestestR::ci(mdl_comb_sim_res, ci=0.95, method="ETI")[1,"CI_low"], bayestestR::ci(mdl_comb_sim_res, ci=0.95, method="ETI")[1,"CI_high"])`  
on the log‐odds scale. Because our model is specified in terms of a **zero–one inflated beta**, this intercept represents the **baseline log‐odds for the beta portion** (i.e., the “regular” responses) when **modality** is *combined*.  

Converting via the inverse‐logit suggests that, ignoring any inflation, the beta part alone predicts **roughly**  
`r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"]))`  
similarity for *combined*.  

However, the **zero–one inflation submodels** add extra probability at **exactly 0 or 1**.  
- The *zoi* submodel’s intercept is  
  `r sprintf("%.2f", fixef(mdl_comb_sim_res, dpar = "zoi")["Intercept","Estimate"])`,  
  which back‐transforms to an inflation probability of  
  `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar = "zoi")["Intercept","Estimate"]))`.  

- The *coi* submodel’s intercept is  
  `r sprintf("%.2f", fixef(mdl_comb_sim_res, dpar = "coi")["Intercept","Estimate"])`,  
  suggesting that a **fraction of observations are fixed at exactly 1**.  

Taking both the beta portion and the inflation components into account, the overall **predicted similarity for combined** is  
`r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"]))`.  

---

**Expressibility**:  
The effect of *expressibility* is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_sim_res)["expressibility_dutch", "Estimate"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[2, "CI_low"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[2, "CI_high"])` on the log-odds scale.  

Higher **expressibility** corresponds to a **higher similarity score**. The predicted similarity at **low expressibility (0.0)** is `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"]))`, whereas at **high expressibility (1.0)**, predicted similarity increases to `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"] + fixef(mdl_comb_sim_res)["expressibility_dutch", "Estimate"]))`.  

This strong positive effect suggests that **concepts with higher expressibility ratings produce guesses that are semantically closer to the intended target**.

---

**Experiment Part**:  
The effect of *experiment part (feedback = Part 2)* is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_sim_res)["exp_part1", "Estimate"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[3, "CI_low"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[3, "CI_high"])` on the log-odds scale.  

Providing **feedback (Part 2)** slightly increases the similarity of guesses compared to no feedback (Part 1). The predicted mean similarity **without feedback (Part 1)** is `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"]))`, while **with feedback (Part 2)**, it rises to `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"] + fixef(mdl_comb_sim_res)["exp_part1", "Estimate"]))`.  

Although the effect of **feedback is statistically credible**, the **absolute change in similarity is minor**, suggesting that feedback has only a limited impact on improving semantic closeness.

---

**Modality**:  
We include *gesture* and *vocal* as predictors in the mu submodel (**combined is the reference**).  

- The **effect of gesture** is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_sim_res)["modalitygesture", "Estimate"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[4, "CI_low"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[4, "CI_high"])`. The **predicted similarity for gesture** is `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"] + fixef(mdl_comb_sim_res)["modalitygesture", "Estimate"]))`. The estimate is **very close to zero**, confirming that **gesture and combined produce nearly identical similarity values**.

- The **effect of vocal** is `r sprintf("%.2f 95%%CI [%.2f, %.2f]", fixef(mdl_comb_sim_res)["modalityvocal", "Estimate"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[5, "CI_low"], bayestestR::ci(mdl_comb_sim_res, ci = 0.95, method = "ETI")[5, "CI_high"])`. The **predicted similarity for vocal** is `r sprintf("%.1f%%", 100 * plogis(fixef(mdl_comb_sim_res, dpar="mu")["Intercept","Estimate"] + fixef(mdl_comb_sim_res)["modalityvocal", "Estimate"]))`. This suggests a **credible but small drop in similarity for vocal**, confirming that **vocal-only guesses are slightly less semantically aligned compared to gesture or combined**.

---

**Conclusion**  
- **Expressibility has the strongest positive impact** on similarity, confirming that more expressive concepts yield **more semantically accurate guesses**.  
- **Experiment part (feedback) has a small but statistically credible effect**—feedback improves correctness slightly but **does not dramatically improve semantic similarity**.  
- **Modality effects are small**:  
  - **Gesture and combined signals perform almost identically**.  
  - **Vocal is slightly worse**, indicating that purely vocal productions lead to **slightly weaker semantic alignment**.  

Overall, the **zero–one inflated beta structure provides a good fit**, as evidenced by the predicted values aligning well with the observed data.


```{r fig.width=10, fig.height=10, fig.cap=capFig("Posterior draws for the fixed effects."), results='hide', warning=FALSE}
# Extract posterior draws for the fixed effects
# For our binary model, the coefficients are on the logit scale.
posterior_fixed <- as.matrix(mdl_comb_sim_res, variable = c("b_Intercept", "b_expressibility_dutch", "b_exp_part1", "b_modalitygesture", "b_modalityvocal"))
...
```{r fig.width=10, fig.height=10, fig.cap=capFig("Posterior estimates of the fixed effects showing the median (dot), 50% (thick line) and 95% (thin lines) quantiles. Also showing 0 as the vertical dotted line and negative (blue) vs positive (red) values."), results='hide', warning=FALSE}
gather_draws(mdl_comb_sim_res, `b_.*`, regex = TRUE) %>%
    # remove the b_ for plotting
    filter(.variable == c("b_Intercept", "b_expressibility_dutch", "b_exp_part1", "b_modalitygesture", "b_modalityvocal")) %>%
    ggplot(aes(x = .value, y = .variable, fill = after_stat(x < 0))) +
    geom_vline(xintercept=0.0, linetype="dotted", color="gray30") +
    stat_halfeye(alpha=0.75, point_interval=median_qi, .width=c(.50, .95)) +
    scale_fill_manual(values = c("lightsalmon", "skyblue"), name=NULL, labels=c("≥ 0", "< 0")) +
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
as.data.frame(VarCorr(mdl_comb_sim_res)$concept$sd) %>%
  rownames_to_column(var = "Random Effect") %>%
  select(`Random Effect`, Estimate, Est.Error, Q2.5, Q97.5)

as.data.frame(VarCorr(mdl_comb_sim_res)$dyad$sd) %>%
  rownames_to_column(var = "Random Effect") %>%
  select(`Random Effect`, Estimate, Est.Error, Q2.5, Q97.5)
```
...
mdl_comb_sim_res %>%
  ...
  geom_pointinterval(aes(xmin = .lower, xmax = .upper),
                     position = position_dodge(width = 0.1)) +
  labs(
    x = "Posterior predicted cosine similarity",
    y = "Concept"
  ) +
  theme_minimal(base_size = 18)
```
...
**Interpretation**:
This zero–one inflated beta model tested how cosine similarity (i.e., the semantic closeness of a guess to the intended target) is influenced by expressibility, experiment part (feedback vs. no feedback), and modality (combined, gesture, vocal).

The baseline intercept (combined modality) is 0.39 on the log-odds scale, corresponding to an estimated mean similarity of about 60% (95% CI [57%, 62%]). This represents the expected similarity score when the guesser is provided with a combined signal, before accounting for expressibility or experiment part effects.

*Expressibility Improves Similarity*
The effect of expressibility (0.59 log-odds) is strongly positive, meaning that more expressive concepts consistently result in guesses that are semantically closer to the intended meaning.
At low expressibility (0.0), the predicted similarity is around 60%.
At high expressibility (1.0), similarity increases to approximately 73% (95% CI [71%, 75%]).
This confirms that concepts that are rated as more expressive lead to more accurate semantic interpretations.

*Feedback Has a Small Positive Effect*
The effect of receiving feedback (Part 2) is small but positive (0.03 log-odds).
Without feedback (Part 1), the predicted similarity is about 60%.
With feedback (Part 2), it increases slightly to 63% (95% CI [61%, 65%]).
This suggests that feedback provides a minor improvement in similarity, but the effect size is relatively modest.

*Modality Effects: Vocal is Slightly Worse than Gesture & Combined*
The gesture coefficient (0.02 log-odds) is very close to zero, meaning that gesture-only signals result in essentially the same similarity scores as combined signals.
The vocal coefficient (−0.16 log-odds) is negative, indicating a small but credible reduction in similarity.
Gesture: ~63% similarity (95% CI [61%, 65%])
Vocal: ~58% similarity (95% CI [56%, 60%])
Posterior hypothesis tests confirm that combined ≈ gesture, both of which are slightly higher than vocal with high posterior certainty.

**Conclusion**
- Expressibility is the strongest predictor: More expressive concepts produce higher guess similarity.
- Feedback provides a small boost: Receiving feedback slightly improves similarity, though the effect is limited.
- Modality effects are small but meaningful:
  - Gesture and combined signals are equally effective.
  - Vocal-only expressions are slightly less transparent, reducing similarity scores by about 5 percentage points compared to gesture or combined signals.
- Thus, while all three modalities allow for relatively successful communication, vocal-only expressions appear slightly less semantically transparent to the guesser.
