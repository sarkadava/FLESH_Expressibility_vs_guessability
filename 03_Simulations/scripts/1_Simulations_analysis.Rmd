...
subtitle: "Preparating data for simulations & analyzing results"
author: Aleksandra Ćwiek, Susanne Fuchs, Wim Pouw, and Šárka Kadavá
date: "2025-06-01"
output:
...
In this script, we prepare a matrix of posterior predictive distributions for each concept in our data. After running simulations with custom Python script, we analyze the results using Bayesian version of t-test.
...
Here we see that the expressibility is not normally distributed, therefore we cannot, for example, sample from normal distribution defined by the mean and standard error whe simulating expressibility of concepts
...
Simulations are run by custom Python script.


# Evaluation of the simulations

After running all simulations, we can return to calculate for each the Bayesian t-test.
...
We loop over each simulation within a simulation type, and perform Bayesian t-test. Then for each simulation type, we assess 1) mean and std of Bayes Factor and proportion of simulations that have BF>3, meaning there is more than anectodal evidence for accepting alternative hypothesis that gesture and vocal expressibility are different.

```{r load in and perform bayes test, warning=FALSE, eval=FALSE}
...
To also assess other information from the Bayesian t-test, we randomly pick one simulation within a simulation type and assess 1) mean and std of the posterior estimate, 2) mean and std of the difference between vocal and gesture expressibility, i.e., beta, 3) mean variance sigma, and 4) mean and std of standardized effect size delta.

```{r sampling posteriors, eval=FALSE, warning=FALSE}
